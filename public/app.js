...
function updateKPIs() {
  ...
  Object.values(usageData.byProject).forEach(group => {
    ...
    totalCached += group.cachedTokens;
    totalConversations += group.conversationsCount;
  });
  
  document.getElementById('kpi-total-cost').innerText = `$${totalCost.toFixed(3)}`;
  document.getElementById('kpi-cost-subtext').innerText = `${totalConversations} conversações ativas`;
...
function renderCharts() {
  ...
  Object.values(usageData.byProject).forEach(group => {
    ...
    projectCosts.push(parseFloat(group.cost.toFixed(3)));
  });
  ...
  // Group tokens by model name
  const modelStats = {};
  
  usageData.rawStats.forEach(conv => {
